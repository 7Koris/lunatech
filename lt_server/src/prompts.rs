...
pub fn select_device_by_prompt(host: &cpal::Host) -> cpal::Device {
    ...
    let prompt = format!(
        "Enter {} for input device, {} for output device",
        "1".green().bold(),
        "2".green().bold()
    );
    ...
    let devices: Vec<cpal::Device> = if input_device == "1" {
        ...
        host.input_devices().unwrap().collect::<Vec<cpal::Device>>()
    } else {
        println!("Select an output device");
        host.output_devices().unwrap().collect::<Vec<cpal::Device>>()
    };
    ...
    let prompt = format!(
        ...
        "0".green().bold(),
        (devices.len() - 1).to_string().green().bold()
    );
